...
class AuthorDaoTest {
    ...
    // using typed queries
    @Test
    void getAuthorsByLastNameLike() {
        // we have it in db
        List<Author> authors = authorDao.getAuthorsByLastNameLike("Wall");
        assertThat(authors).isNotNull();
        assertThat(authors.size()).isGreaterThan(0);
        authors.forEach(author -> System.out.println(author.getId() + " " + author.getLastName() + " " + author.getFirstName()));
    }
    ...
    @Test
    void getAuthorByNameNamedQuery() {
        Author author = authorDao.getAuthorByNameNamedQuery("Craig", "Walls");
        assertThat(author).isNotNull();
        assertEquals("Craig", author.getFirstName());
        assertEquals("Walls", author.getLastName());
        System.out.println(author.getFirstName() + " " +  author.getLastName());
